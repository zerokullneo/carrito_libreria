/* $Id: test-caso1-consola.cpp 316 2016-04-15 19:26:32Z gerardo $
...
int main()
{
  ...
  Usuario
    ...
    sanani("sanani", "Sanani", "de Escobas",
	  "Callejón de los negros, s/n (Cádiz)", "ahxo2Aht"),
    rosa("rosa", "Rosita", "de Jerico",
  ...
  try {		 // O938 en vez de 0938 ;-) (era una O, no un cero)
    Tarjeta falsa(Tipo::AmericanExpress, Numero("4539 4512 O398 7356"), lucas, "31/12/2020");
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta judas(Tipo::AmericanExpress, Numero("1234567890123456"), lucas, "31/12/2020");
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  ...
  try {
    Tarjeta caduca(Tipo::AmericanExpress, Numero("4222222222222"), lucas, "30/04/2002");
  }
  catch(Tarjeta::Caducada &e) {
    std::cerr << "Error: tarjeta caducada. " << e.cuando() << std::endl;
  }
  try { 			// En la fecha, O = letra O, no cero (O-0)
    Tarjeta rota(Tipo::AmericanExpress, Numero(" 4222222222222 "), lucas, "1O/O4/2O2O");
  }
  ...
  mostrar_carro(std::cout, sanani);
  std::cout << "\n*** sanani devuelve 4 ADA" << std::endl;
